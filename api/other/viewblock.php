...
class getInfo implements igetInfo
{
	public function getInfos()
	{
		...
		$forumarchive = array();
		// $arr = C::t('forum_forum')->fetch_all_fids();
		// $arr = C::t('forum_forumfield')->fetch_all_by_fid();
		// $_G['fid'] = 1;
		//while($result = C::t('forum_forum')->fetch_all_info_by_fids($fup=1)){
		//操作forum_forum表查询板块名，发帖数量
		foreach(C::t('forum_forum')->fetch_all_fids() as $archive){
			$forumarchive[$archive['fid']] = array(
				'moderators' => '',				
				'name' => $archive['name'],
				'posts' => $archive['posts']
			);
		...
		foreach($forumarchive as $k=>$v){
		//操作forum_forumfield表查询板块作者
			foreach(C::t('forum_forumfield')->fetch_all_by_fid($k) as $arch){
				$forumarchive[$k]['moderators'] = $arch['moderators'];
			}
		}
		//返回forumarchive数组值
		return $forumarchive;
